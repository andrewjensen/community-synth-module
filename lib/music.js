const Music = (function() {
  ...
  function snapToNearestAllowed(noteValue, mode) {
    const allowedNotes = MODES[mode].allowed;
    if (isAllowed(noteValue, allowedNotes)) {
      // This note is already allowed.
      return noteValue;
    }

    // This note is not allowed, so pick the closest allowed note.
    const surrounding = getSurroundingNotes(noteValue, mode);
    const lowerNote = surrounding.lowerNote;
    const higherNote = surrounding.higherNote;
    if ((noteValue - lowerNote) <= (higherNote - noteValue)) {
      // lower is less distance, or they are equal distance.
      return lowerNote;
    } else {
      return higherNote;
    }
